...
invest_renew_df <- read.csv("../preprocessed/invest_to_renewable.csv")
renew_gdp_pop_df <- read.csv("../preprocessed/renew_gdp_population.csv")
invest_dev_df <- read.csv("../preprocessed/invest_developing_countries.csv")
num_business_df <- read.csv("../preprocessed/num_business.csv")
industrial_production_df <- read.csv("../preprocessed/industrial_production_growth_rate.csv")
```
...
gdp_2017 <- renew_gdp_pop_df %>% 
  plot_ly(x = ~gdp_2017, y = ~renew_ratio_2017, text=~country, name='2017') %>% 
  layout(title='GDP vs. Renewable Energy Ratio', xaxis=x, yaxis=y)

gdp_2018 <- renew_gdp_pop_df %>% 
  plot_ly(x = ~gdp_2018, y = ~renew_ratio_2018, text=~country, name='2018') %>% 
  layout(xaxis=x, yaxis=y)

subplot(gdp_2017, gdp_2018, titleY=TRUE, shareY=TRUE, titleX=TRUE)
```
```{r}
...
gdp_capita_2017 <- renew_gdp_pop_df %>% 
  plot_ly(x = ~gdp_per_capita_2017, y = ~renew_ratio_2017, text=~country, name='2017') %>% 
  layout(title='GDP per Capita vs. Renewable Energy Ratio', xaxis=x, yaxis=y)

gdp_capita_2018 <- renew_gdp_pop_df %>% 
  plot_ly(x = ~gdp_per_capita_2018, y = ~renew_ratio_2018, text=~country, name='2018') %>% 
  layout(xaxis=x, yaxis=y)

subplot(gdp_capita_2017, gdp_capita_2018, titleY=TRUE, shareY=TRUE, titleX=TRUE)
```
...
num_business_df %>% 
  plot_ly(x = ~num_business_2018, y = ~renew_ratio_2018, text=~country) %>% 
  layout(title='Business Count and Renewable Energy Ratio 2018', xaxis=x, yaxis=y)
```
...
There seems to be a weak correlation between the number of businesses in a country and the renewable energy ratio. This may be because the number of businesses have an affect on how much people are making profit, in other words, the GDP per capita. 
...
```{r}
x <- list(
  title = "Industrial Production Growth Rate"
)
...
industrial_production_df %>% 
  plot_ly(x = ~production_growth_rate_2017, y = ~renew_ratio_2017, text=~country) %>% 
  layout(title='Industrial Production Growth Rate and Renewable Energy Ratio 2017', xaxis=x, yaxis=y)
```
